...
void main() {
  runApp(const MaterialApp(home: Status()));
}
`;

// The lifecycle sign-off: useEffect -> initState, ternary children, and
// inline setState handlers — TSX in, web build out.
describe('mount-effect TSX builds as a real Flutter app', () => {
  test('fixture #6 input.tsx transpiles and compiles into a web build', async () => {
    ...
    await Bun.write(join(appDir, 'lib', 'status.dart'), generated);
