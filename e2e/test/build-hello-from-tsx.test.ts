...
void main() {
  runApp(const MaterialApp(home: HelloScreen()));
}
`;

// The first GREEN from-TSX pipeline: stateless TSX in, running web build out.
describe('stateless TSX builds as a real Flutter app', () => {
  test('fixture #2 input.tsx transpiles and compiles into a web build', async () => {
    ...
    await Bun.write(join(appDir, 'lib', 'hello_screen.dart'), generated);
