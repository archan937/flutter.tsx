...
void main() {
  runApp(const MaterialApp(home: TaskBoard()));
}
`;

// The typed-props sign-off: named prop interfaces, fragments, final fields —
// TSX in, web build out.
describe('typed-props TSX builds as a real Flutter app', () => {
  test('fixture #9 input.tsx transpiles and compiles into a web build', async () => {
    ...
    await Bun.write(join(appDir, 'lib', 'tasks.dart'), generated);
