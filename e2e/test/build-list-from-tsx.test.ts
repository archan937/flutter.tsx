...
void main() {
  runApp(const MaterialApp(home: Groceries()));
}
`;

// The list-rendering sign-off: .map() -> collection-for, TSX in, web build out.
describe('list-rendering TSX builds as a real Flutter app', () => {
  test('fixture #7 input.tsx transpiles and compiles into a web build', async () => {
    ...
    await Bun.write(join(appDir, 'lib', 'groceries.dart'), generated);
