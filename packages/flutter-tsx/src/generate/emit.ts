...
const CHILDREN_TYPES: Record<ChildrenSlot['kind'], string> = {
...
const propLine = (
  ...
  const slotType = slot?.mode === 'multi' ? 'FlutterChild[]' : 'FlutterChild';
  const tsType =
    slot !== undefined
      ? slotType
      : tsTypeOf(param.required ? param.type : stripOuterNullable(param.type));
...
const childrenLine = (children: ChildrenSlot, params: ParamModel[]): string => {
  ...
  const declaration = `  children${optional}: ${CHILDREN_TYPES[children.kind]};`;
  return jsdoc === '' ? declaration : `${jsdoc}\n${declaration}`;
};
